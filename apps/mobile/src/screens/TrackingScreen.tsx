...
export function TrackingScreen({ route }: { route: { params: { orderId: string } } }) {
  ...
  useEffect(() => {
    ...
    // networks, and 10s is well inside what feels live for a 30-minute delivery.
    ...
    <View style={styles.container}>
      ...
      {tracking.courier ? (
        <View style={styles.courierCard}>
          ...
          {/* In-app calling keeps both phone numbers private — the customer and
              courier are bridged, never given each other's number. */}
          <Pressable style={styles.callButton}>
            <Text style={styles.callButtonText}>Kurier anrufen</Text>
          </Pressable>
        </View>
      ) : null}
...
const styles = StyleSheet.create({
  ...
  reference: { ...theme.type.caption, color: theme.colors.textMuted },
  ...
  cancelButton: { marginTop: "auto", alignItems: "center", paddingVertical: theme.spacing(2) },
